...
function currentCity(event) {
  ...
  let selectedCityName = cityTimeZone.replace("_", " ").split("/")[1];
  let cityBox = document.querySelector("#city-box");
  cityBox.innerHTML = `
        <div class="row">
          <div class="col-6">
            <div class="city-name">${selectedCityName}</div>
            <div class="city-date">${selectedCityTime.format(
              "dddd, MMMM Do, YYYY"
            )}
              </div>
              </div>
            <div class="col-6">
            <div class="city-time">${selectedCityTime.format("h:mm:ss A")}</div>
          </div>
        </div>
...
citySelect.addEventListener("change", currentCity);
